...
% Float placement. With the defaults, Figure 2's block (figure + its long caption,
% ~55% of \textheight) clears \floatpagefraction=0.5, so LaTeX preferred a float-only
% page and centred it with \@fptop/\@fpbot fill glue -- stranding it between two blank
% bands. Raising \floatpagefraction above that block's share disqualifies the sparse
% float page; the larger \topfraction then lets it ride at the top of a text page like
...
\date{Preprint, July 2026 \textbullet\ all analyses use public data and are fully scripted and reproducible}
