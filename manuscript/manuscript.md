...
reading frames (ncORFs), found in long non-coding RNAs, pseudogenes, 5′/3′ untranslated regions, and
...
presentation); and T cells must genuinely respond. As the field moves toward the clinic, one
prerequisite question, to our knowledge unmeasured, is whether the claims already in the
...
correspond to a 36% FDR for the novel-peptide class while the canonical class sat at 0.03% [19].
...
largest public cryptic-peptide atlases. We report two findings. (i) A direct measurement: **56.3% of
...
candidate-space ratio (in a regime we characterize), and, propagated to the claim level, a
calibrated per-claim truth probability is **not identifiable**; the honest output is a partial
order over claims. **Third**, we give the minimal fix: a per-claim
reporting standard that chiefly asks for the per-class decoy count `D_N` on the claim's unit. It
...
spectrum), whatever ORF they are attributed to. This is **≈800× a composition-matched shuffle null** (Fig 1),
...
recognised, and canonical-subtraction is a real, adoptable safeguard: CrypticProteinDB's own
protein-level BLASTP canonical-exclusion filter (below) is a concrete, already-implemented example
...
≈1×, i.e. no enrichment; these are genuinely non-canonical). This is not in tension with the 88.8%
...
It is the same frame distinction, not a contradiction between "altORF" classes, so the contamination has a
...
own; a claim carrying both the ledger and a reusable assay result would strict-pass all four, and the
gap is unmet reporting, not an impossible standard.

### Where the evidence package breaks down
The 0% survival is forced jointly by two independently-zero axes, source-ORF and immunogenicity, not
by knob-twisting on one (Fig 2). Source-ORF translation rigour is reported, per claim, for ≈0% of the
corpus, and the immunogenicity axis is empty of machine-readable human positives; either alone forces
the joint result to 0%. The HLA-presentation axis compounds this further with its own allele-assignment
cliff: 99.96% of unique peptides (all but ≈80) carry no per-peptide HLA allele in reusable form. Across both
...
identified set** `Θ_N(q,f) = [ max(0,(q-(1-f))/f), min(1, q/f) ]`. This is the same mixture-bound construction
as the classical ecological-inference "method of bounds" [23], applied here to a setting that literature
never had: the class rate is *directly observable* via the accepted decoy count (below), so the
...
1% [19]; recent pipelines built on this lesson use separate class-specific FDR by design [20]). That the
realized value can be high yet remain invisible to the reported global figure is
...
though, every claim's *joint* probability still has lower bound 0. This is not because of the M-marginal, but
...
chooses the rule [18]. On the present data this rarely licenses acceptance, since class-FDR lower bounds are zero, so
...
target-decoy threshold. Proteogenomics, rare post-translational modifications, and any setting where a global
multiple-testing threshold is applied to a small relevant subset all qualify, so the contribution is the publication-record
...
converts a class of claims that today cannot be independently checked into one that can be, on the three axes
reporting alone can fix; the fourth, immunogenicity, still needs its own reusable assay report.
...
counts plus the null law; neither is in the reported record. This assumes a single null `F` shared across
...
than derived. It likewise assumes the accepted-decoy-generating spectra are compositionally
representative of the full searched set: because `μ_g` sums over all spectra, a compositionally biased
null subset would make the class split a weighted average over an unidentified subset rather than the
global `ρ_eff`.
...
7. Keich U, Kertesz-Farkas A, Noble WS. Improved false discovery rate estimation procedure for shotgun proteomics. *J Proteome Res* 2015;14(8):3148–3161. doi:10.1021/acs.jproteome.5b00081. (PMC4533616): rigorous target-decoy FDR estimation (T-TDC, C-TDC).
